...
class Team:
    ...
    def change_pitcher(self, new_pitcher):
        ...
        if old_pitcher is new_pitcher:
            return False, f"{new_pitcher.name} is already the current pitcher"
        ...
        # 古い投手を退場リストに追加
        if old_pitcher and old_pitcher not in self.ejected_players:
    ...
    def get_available_pitchers(self):
        ...
        for pitcher in self.pitchers:
            if pitcher == self.current_pitcher:
                # 既に登板中の投手は交代候補に含めない
                continue
            if self.is_player_eligible(pitcher):
    ...
    def validate_defensive_positions(self):
        ...
        assigned_positions = {pos: player for pos, player in self.defensive_positions.items() if player is not None}
