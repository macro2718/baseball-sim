...
class SubstitutionManager:
    ...
    def validate_substitution(
        ...
        *,
        ...
        """
        if substitute_player in getattr(self.team, "ejected_players", []):
            return False, f"{substitute_player.name} has already been ejected and cannot re-enter"
        
        if substitute_player in getattr(self.team, "lineup", []):
            return False, f"{substitute_player.name} is already in the game"
        
        available_bench = []
        if hasattr(self.team, "get_available_bench_players"):
            available_bench = self.team.get_available_bench_players()
        elif hasattr(self.team, "bench"):
            available_bench = [p for p in self.team.bench if p not in self.team.ejected_players]
        
        available_pitchers = []
        if hasattr(self.team, "get_available_pitchers"):
            available_pitchers = self.team.get_available_pitchers()
        elif hasattr(self.team, "pitchers"):
            available_pitchers = [p for p in self.team.pitchers if p not in self.team.ejected_players]
        
        is_bench_player = substitute_player in available_bench
        is_pitcher = substitute_player in available_pitchers

        if not (is_bench_player or is_pitcher):
            # 投手リストに存在する場合は投手としては出場不可、それ以外はベンチ要員として出場不可
            if hasattr(self.team, "pitchers") and substitute_player in self.team.pitchers:
                return False, f"{substitute_player.name} is not eligible to pitch"
    ...
    def execute_pitcher_change(self, pitcher_index: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 妥当性チェック
            valid, message = self.validate_substitution(new_pitcher, position="P")
            if not valid:
                return False, message
    ...
    # Removed unused format_player_info helper.
    
    # Removed unused helpers related to individual position changes, summary info,
    # and formatted lists for CLI output.
