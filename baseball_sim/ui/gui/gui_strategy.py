...
class StrategyManager:
    ...
    # 統一化されたメソッド群
    def _pinch_hit_unified(self, bench_index, substitution_manager, parent_window=None):
        """統一化された代打処理"""
        # ゲーム終了チェック
        if self._is_game_over():
            messagebox.showinfo("Game Over", "The game has ended. No more strategies can be executed.")
    ...
    def _change_pitcher_unified(self, pitcher_index, substitution_manager, parent_window=None):
        """統一化された投手交代処理"""
        # ゲーム終了チェック
        if self._is_game_over():
            messagebox.showinfo("Game Over", "The game has ended. No more strategies can be executed.")
